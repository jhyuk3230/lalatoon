      ...
      <ul className="border-t border-black">
				{ list.map((e, i) => (
					<li className={`border-b border-gray-100 ${read.includes(e.id) ? "bg-gray-50" : ""} ${expiration && !collectEpisode?.includes(e.id) ? "opacity-30 pointer-events-none" : ""}`} key={i}>
						<button data-link={`${expiration ? "" : "#"}`} className="w-full py-[10px] flex justify-between items-center text-left" onClick={() => {episodeOnClick(e.id)}}>
							<div className="flex justify-start items-center gap-[15px]">
								...
								<div className="flex flex-col">
									...
									<h4 className="flex flex-start items-center gap-1 text-[14px] font-bold text-black leading-[20px]">
										{e.new ? (
											<span className="px-1 py-1 rounded-[20px] inline-block bg-red-500 text-[8px] font-bold text-white leading-[1]">NEW</span>
										) : ""}
							...
							<div className="w-[90px] flex flex-col justify-center items-center flex-shrink-0">
								{ collectEpisode?.includes(e.id) ? (
									<p className="text-[13px] font-bold text-gray-400 text-center">소장</p>
								) : (
									<>
										{read.includes(e.id) ? (
											<>
												{e.id == lastRead ? (
													<>
														<p className="text-[13px] font-bold text-gray-400 text-center">Recently<br />Read</p>
													</>
												) : (
													<>
														<p className="text-[13px] font-bold text-gray-400">Read</p>
													</>
												)}
											</>
										) : (
											<>
												{e.free ? (
													<p className="text-[13px] font-bold text-gray-400 text-center">무료</p>
												) : (
													<>
														{ e.waitFree ? (
